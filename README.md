...
### 01. Python
To use the Python version, you could download it and run it from a code editor or terminal. _Make sure to turn ON your Caps Lock._ It's simple to understand. There are 5 options: _ASSEMBLE, REGISTER VIEW, SINGLE STEP, GO EXECUTE and MEMORY VIEW/EDIT_.
...
And that's it. That's all you need to know to write and execute an ALP.
...
To enter an ALP, you should press '_SUB_' and type your starting address and press '_NEXT_'. Enter your opcode and press '_NEXT_' and so on. After you've entered your opcode, press '_RST_' to exit. The procedure is the same if you want to write data to or read data from the RAM.

To execute/run a program, press '_GO_' and type your starting address and press '_EXEC_'. And then '_RST_'. As with the LCD simulator, if you're running the website on a PC, press '_Ctrl + Shift + I_' or '_Settings > Developer Tools_' and go to '_console_' to see the program doing its thing.

To view register contents, press the '_REG_' button and press the bottom buttons which have '_A_', '_B_', '_C_', '_D_', '_E_', '_H_', '_L_'. Press on each one to see its contents.
...
Updated on 11/10/2023, Wednesday ( Jump instructions, increment, decrement work. Added LDA, STA, SUB, SUI instructions ).
...
Updated on 19/10/2023, Thursday ( Made some changes to the memory function ).
